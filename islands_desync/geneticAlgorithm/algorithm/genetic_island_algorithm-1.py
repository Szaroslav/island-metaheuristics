...
class GeneticIslandAlgorithm(GeneticAlgorithm):
    def __init__(
        ...
        self.tab_jakosc_migracji_z_wysp = [[0 for i in range (20)], [0 for i in range(20)]]
    ...
    def add_new_individuals(self):
        ...
        #print("--------------------", imigr_list, imigr_ftnss_list, imigr_iter_list)

        #if self.island == 12:
            #print("W",self.island,"[",self.step_num,"] listy: imigr: ", imigr_list,type(imigr_list), "fitnesy", imigr_ftnss_list, type(imigr_ftnss_list),"iteracje", imigr_iter_list,type(imigr_iter_list), "LEN imigr_list", len(imigr_list)) #,"LEN imigr_list[0]", len(imigr_list[0]))
            #print("W",self.island,"[",self.step_num,"] przed   ",self.tab_jakosc_migracji_z_wysp)
        ...
        for imigr_ind in range (len(imigr_list)):
                ...
                #print("W",self.island, "[",self.step_num,"] imigr ind ---->",imigr_ind, "plusuje",imigr_list,imigr_ind ,imigr_list[imigr_ind])
            ...
            if imigr_ftnss_list[imigr_ind]<self.lastBest:
                ...
                #skopiuj do tych co mają zostac i spr ranking wyspy z kt prxyszedl !!!!!!!!!!!!!!!!!!!!
        ...
        if (self.last_step - 1 < self.step_num) and not self.bylLog:
            ...
            for ind in range(len(self.tab_jakosc_migracji_z_wysp[0])):
                if self.tab_jakosc_migracji_z_wysp[0][ind]==0:
                    tab_procent.append(0)
                else:
                    tab_procent.append(round( self.tab_jakosc_migracji_z_wysp[1][ind] / self.tab_jakosc_migracji_z_wysp[0][ind] * 100,2))
